...
pub const SENSOR_ADDRESS: u8 = 0x5D;
...
pub const EVEN_ROW_EVEN_COLUMN_GAIN: u8 = 0x2B;
pub const ODD_ROW_EVEN_COLUMN_GAIN: u8 = 0x2C;
pub const EVEN_ROW_ODD_COLUMN_GAIN: u8 = 0x2D;
pub const ODD_ROW_ODD_COLUMN_GAIN: u8 = 0x2E;
pub const TEST_DATA: u8 = 0x32;
pub const GLOBAL_GAIN: u8 = 0x35;
pub const CAL_THRESHOLD: u8 = 0x5F;
pub const EVEN_ROW_EVEN_COLUMN_ANALOG_OFFSET: u8 = 0x60;
pub const ODD_ROW_ODD_COLUMN_ANALOG_OFFSET: u8 = 0x61;
pub const CAL_CTRL: u8 = 0x62;
pub const EVEN_ROW_ODD_COLUMN_ANALOG_OFFSET: u8 = 0x63;
pub const ODD_ROW_EVEN_COLUMN_ANALOG_OFFSET: u8 = 0x64;
...
impl OutputControl {
    ...
    pub fn set_synchronize_changes(mut self, value: bool) -> Self {
    ...
    /// FRAME_VALID.)
    /// 1 = do not update any changes to these settings until this bit is returned to "0."
    pub fn get_synchronize_changes(&self) -> bool {
        (self.value & 1u16 << 0) != 0
    }
    ...
    pub fn set_chip_enable(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 1;
        self
    }
    ...
    /// a new frame. The digital power consumption can then also be reduced to less than 5uA by turning
    /// off the master clock.
    pub fn get_chip_enable(&self) -> bool {
        (self.value & 1u16 << 1) != 0
    }
    ...
    pub fn set_use_test_data(mut self, value: bool) -> Self {
    ...
    /// the same width, height, and frame rate as it would otherwise have. No digital processing (gain or
    /// offset) is applied to the data. When clear (the default), sampled pixel values are output normally
    pub fn get_use_test_data(&self) -> bool {
        (self.value & 1u16 << 6) != 0
    }
}
...
impl ReadOptions1 {
    ...
    pub fn set_column_skip_4(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 2;
        self
    }

    /// ///Column Skip 4-default is 0 (disable). 1 = enable.
    pub fn get_column_skip_4(&self) -> bool {
        (self.value & 1u16 << 2) != 0
    }

    /// ///Row Skip 4-default is 0 (disable). 1 = enable.
    pub fn set_row_skip_4(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 3;
        self
    }

    /// ///Row Skip 4-default is 0 (disable). 1 = enable.
    pub fn get_row_skip_4(&self) -> bool {
        (self.value & 1u16 << 3) != 0
    }

    /// ///Column Skip 8-default is 0 (disable). 1 = enable.
    pub fn set_column_skip_8(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 4;
        self
    }

    /// ///Column Skip 8-default is 0 (disable). 1 = enable.
    pub fn get_column_skip_8(&self) -> bool {
        (self.value & 1u16 << 4) != 0
    }

    /// ///Row Skip 8-default is 0 (disable). 1 = enable.
    pub fn set_row_skip_8(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 5;
        self
    }

    /// ///Row Skip 8-default is 0 (disable). 1 = enable.
    pub fn get_row_skip_8(&self) -> bool {
        (self.value & 1u16 << 5) != 0
    }
    ...
    pub fn set_snapshot_mode(mut self, value: bool) -> Self {
    ...
    /// 1 = enable (wait for TRIGGER; TRIGGER can come from outside signal (TRIGGER pin on the sensor)
    /// or from serial interface register restart, i.e. programming a "1" to bit 0 of Reg0x0B
    pub fn get_snapshot_mode(&self) -> bool {
        (self.value & 1u16 << 8) != 0
    }
    ...
    pub fn set_strobe_enable(mut self, value: bool) -> Self {
    ...
    /// 1 = enable STROBE (signal output from the sensor during the time all rows are integrating. See
    /// STROBE width for more information).
    pub fn get_strobe_enable(&self) -> bool {
        (self.value & 1u16 << 9) != 0
    }
    ...
    pub fn set_strobe_width(mut self, value: bool) -> Self {
    ...
    /// prior to line valid going HIGH)
    /// 1 = extend STROBE width (STROBE signal width extends to entire time all rows are integrating).
    pub fn get_strobe_width(&self) -> bool {
    ...
    pub fn set_strobe_override(mut self, value: bool) -> Self {
    ...
    /// 1 = override STROBE signal (STROBE signal is set HIGH when this bit is set, LOW when this bit is set
    /// LOW. It is assumed that STROBE enable is set to "0" if STROBE override is being used).
    pub fn get_strobe_override(&self) -> bool {
...
impl ReadOptions2 {
    ...
    pub fn set_no_bad_frames(mut self, value: bool) -> Self {
        self.value |= value as u16;
        self
    }
    ...
    /// ///Flip Row-1 = readout starting 1 row later (alternate color pair)
    /// 0 (default) = normal readout.
    pub fn get_flip_row(&self) -> bool {
        (self.value & 1u16 << 7) != 0
    }

    /// ///1 = "Continuous" LINE_VALID (continue producing LINE_VALID during vertical blanking).
    pub fn set_continuous_line_valid(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 9;
        self
    }

    /// ///1 = "Continuous" LINE_VALID (continue producing LINE_VALID during vertical blanking).
    pub fn get_continuous_line_valid(&self) -> bool {
        (self.value & 1u16 << 9) != 0
    }

    /// ///1 = LINE_VALID = "Continuous" LINE_VALID XOR FRAME_VALID.
    /// 0 = LINE_VALID determined by bit 9.
    pub fn set_continuous_line_valid_xor_frame_valid(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 10;
        self
    }
    ...
    /// ///1 = LINE_VALID = "Continuous" LINE_VALID XOR FRAME_VALID.
    /// 0 = LINE_VALID determined by bit 9.
    pub fn get_continuous_line_valid_xor_frame_valid(&self) -> bool {
        (self.value & 1u16 << 10) != 0
    }

    /// ///the black rows can also be read out by setting the sensor to raw data output mode.
    pub fn set_raw_data_output_mode(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 11;
        self
    }

    /// ///the black rows can also be read out by setting the sensor to raw data output mode.
    pub fn get_raw_data_output_mode(&self) -> bool {
        (self.value & 1u16 << 11) != 0
    }

    /// ///Mirror Row-1 = read out from bottom to top (upside down).
    /// 0 (default) = normal readout (top to bottom).
    pub fn set_mirror_row(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 15;
        self
    }

    /// ///Mirror Row-1 = read out from bottom to top (upside down).
    /// 0 (default) = normal readout (top to bottom).
    pub fn get_mirror_row(&self) -> bool {
...
impl CalThreshold {
    ...
    pub fn set_thres_lo(mut self, value: u16) -> Self {
        let mask = !0u16 << 5;
    ...
    pub fn set_override_automatic_thres_hi_and_thres_lo_adjust(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 7;
        self
    }
    ...
    /// ///1 = override automatic Thres_hi and Thres_lo adjust (Thres_hi always = bits 14:8; Thres_lo always = bits 5:0).
    /// Default = 0 = Automatic Thres_hi and Thres_lo adjustment.
    pub fn get_override_automatic_thres_hi_and_thres_lo_adjust(&self) -> bool {
        (self.value & 1u16 << 7) != 0
    }
    ...
    pub fn set_thres_hi(mut self, value: u16) -> Self {
        let mask = (!0u16 << 14) | ((1 << 8) - 1);
    ...
    /// Black level maximum is set to this value when bit 7 = 1; black level maximum is reset to this value
    /// after every black level average restart if bit 15 = 1 and bit 7 = 0.
    pub fn get_thres_hi(&self) -> u16 {
        self.value >> 8 & (0xFFFF << (14 - 8))
    }
    ...
    pub fn set_no_gain_dependence(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 15;
        self
    }
    ...
    /// (bits 14:8) after every black level average restart.
    /// 0 = Thres_lo and Thres_hi are set automatically, as described above.
    pub fn get_no_gain_dependence(&self) -> bool {
        (self.value & 1u16 << 15) != 0
    }
}
...
impl CalCtrl {
    ...
    pub fn set_manual_override_of_black_level_correction(mut self, value: bool) -> Self {
        self.value |= value as u16;
        self
    }
    ...
    /// 1 = override automatic black level correction with programmed values.
    /// 0 = normal operation (default).
    pub fn get_manual_override_of_black_level_correction(&self) -> bool {
        (self.value & 1u16 << 0) != 0
    }

    /// ///disable black level correction (Offset Correction Voltage = 0.0V).
    pub fn set_disable_black_level_correction(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 1;
        self
    }

    /// ///disable black level correction (Offset Correction Voltage = 0.0V).
    pub fn get_disable_black_level_correction(&self) -> bool {
        (self.value & 1u16 << 1) != 0
    }

    /// ///0 = apply black level calibration during ADC operation only (default).
    /// 1 =  apply black level calibration continuously.
    pub fn set_apply_black_level_calibration_continuously(mut self, value: bool) -> Self {
        self.value |= (value as u16) << 2;
        self
    }
    ...
    /// ///0 = apply black level calibration during ADC operation only (default).
    /// 1 =  apply black level calibration continuously.
    pub fn get_apply_black_level_calibration_continuously(&self) -> bool {
        (self.value & 1u16 << 2) != 0
    }
    ...
    pub fn set_do_not_reset_the_upper_threshold_after_a_black_level_recalculation_sweep(
        mut self,
        value: bool,
    ) -> Self {
        self.value |= (value as u16) << 11;
    ...
    /// ///1 = do not reset the upper threshold after a black level recalculation sweep.
    /// 0 = reset the upper threshold after a black level recalculation sweep (default).
    pub fn get_do_not_reset_the_upper_threshold_after_a_black_level_recalculation_sweep(
        &self,
    ) -> bool {
        (self.value & 1u16 << 11) != 0
    }
    ...
    /// 0 = normal operation (default).
    pub fn set_start_a_new_running_digitally_filtered_average_for_the_black_level(
        mut self,
        value: bool,
    ) -> Self {
    ...
    /// immediately), and do a rapid sweep to find the new starting point.
    /// 0 = normal operation (default).
    pub fn get_start_a_new_running_digitally_filtered_average_for_the_black_level(&self) -> bool {
        (self.value & 1u16 << 12) != 0
    }

    /// ///1 = do not perform the rapid black level sweep on new gain settings.
    /// 0 = normal operation.
    pub fn set_do_not_perform_the_rapid_black_level_sweep_on_new_gain_settings(
        mut self,
        value: bool,
    ) -> Self {
        self.value |= (value as u16) << 15;
    ...
    /// ///1 = do not perform the rapid black level sweep on new gain settings.
    /// 0 = normal operation.
    pub fn get_do_not_perform_the_rapid_black_level_sweep_on_new_gain_settings(&self) -> bool {
        (self.value & 1u16 << 15) != 0
    }
}
    ...
    RW: I2c,
{
    ...
    pub fn get_chip_version(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(CHIP_VERSION)
    }

    /// ///This register is read-only and gives the chip identification number: 0x8431 (1000 0100 0001 0001).
    pub fn set_chip_version(&mut self, value: u16) -> Result<(), RW::Error> {
        self.set_u16(CHIP_VERSION, value)
    }

    /// ///First row to be read out-default = 0x000C (12). Data format: 0000 0ddd dddd dddd
    pub fn get_row_start(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(ROW_START)
    }

    /// ///First row to be read out-default = 0x000C (12). Data format: 0000 0ddd dddd dddd
    pub fn set_row_start(&mut self, value: u16) -> Result<(), RW::Error> {
    ...
    pub fn get_column_start(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(COLUMN_START)
    }

    /// ///First column to be read out-default = 0x0014 (20).
    /// Register value must be an even number. Data format: 0000 0ddd dddd dddd
    pub fn set_column_start(&mut self, value: u16) -> Result<(), RW::Error> {
    ...
    pub fn get_row_size(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(ROW_SIZE)
    }

    /// ///Window height (number of rows - 1)-default = 0x03FF (1023).
    /// Minimum value for 0x03 = 0x0002. Data format: 0000 0ddd dddd dddd
    pub fn set_row_size(&mut self, value: u16) -> Result<(), RW::Error> {
    ...
    pub fn get_column_size(&mut self) -> Result<u16, RW::Error> {
    ...
    /// Register value must be an odd number.
    /// Minimum value for 0x04 = 0x0003. Data format: 0000 0ddd dddd dddd
    pub fn set_column_size(&mut self, value: u16) -> Result<(), RW::Error> {
        self.set_u16(COLUMN_SIZE, value)
    }

    /// ///Horizontal Blanking-default = 0x0009 (9 pixels). Data format: 0000 0ddd dddd dddd
    pub fn get_horizontal_blanking(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(HORIZONTAL_BLANKING)
    }

    /// ///Horizontal Blanking-default = 0x0009 (9 pixels). Data format: 0000 0ddd dddd dddd
    pub fn set_horizontal_blanking(&mut self, value: u16) -> Result<(), RW::Error> {
        self.set_u16(HORIZONTAL_BLANKING, value)
    }

    /// ///Vertical Blanking-default = 0x0019 (25 rows). Data format: 0000 0ddd dddd dddd
    pub fn get_vertical_blanking(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(VERTICAL_BLANKING)
    }

    /// ///Vertical Blanking-default = 0x0019 (25 rows). Data format: 0000 0ddd dddd dddd
    pub fn set_vertical_blanking(&mut self, value: u16) -> Result<(), RW::Error> {
    ...
    pub fn get_output_control(&mut self) -> Result<OutputControl, RW::Error> {
        Ok(OutputControl::new(self.get_u16(OUTPUT_CONTROL)?))
    }

    /// ///This register controls various features of the output format for the sensor. Data format: 0000 0000 0d00 00dd
    pub fn set_output_control(&mut self, value: &OutputControl) -> Result<(), RW::Error> {
        self.set_u16(OUTPUT_CONTROL, value.value)
    }

    /// ///Number of rows of integration-default = 0x0419 (1049). Data format: 00dd dddd dddd dddd
    pub fn get_shutter_width(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(SHUTTER_WIDTH)
    }

    /// ///Number of rows of integration-default = 0x0419 (1049). Data format: 00dd dddd dddd dddd
    pub fn set_shutter_width(&mut self, value: u16) -> Result<(), RW::Error> {
    ...
    pub fn get_restart(&mut self) -> Result<u16, RW::Error> {
    ...
    /// Reg0x20, bit0).
    /// Data format: 0000 0000 0000 000d
    pub fn set_restart(&mut self, value: u16) -> Result<(), RW::Error> {
        self.set_u16(RESTART, value)
    }
    ...
    pub fn get_shutter_delay(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(SHUTTER_DELAY)
    }

    /// ///Shutter delay-default = 0x0000 (0). This is the number of master clocks times four that the timing
    /// and control logic waits before asserting the reset for a given row. Data format: 0000 0ddd dddd dddd
    pub fn set_shutter_delay(&mut self, value: u16) -> Result<(), RW::Error> {
    ...
    /// Data format: 0000 0000 0000 000d
    pub fn get_reset(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(RESET)
    }
    ...
    /// mode first write a "1" into bit 0 of this register, then write a "0" into bit 0 to resume operation.
    /// Data format: 0000 0000 0000 000d
    pub fn set_reset(&mut self, value: u16) -> Result<(), RW::Error> {
    ...
    pub fn get_read_options_1(&mut self) -> Result<ReadOptions1, RW::Error> {
        Ok(ReadOptions1::new(self.get_u16(READ_OPTIONS_1)?))
    }

    /// ///In read mode 1, this register is used to control many aspects of the readout of the sensor. Data format: 1000 dddd 00dd dd00
    pub fn set_read_options_1(&mut self, value: &ReadOptions1) -> Result<(), RW::Error> {
    ...
    pub fn get_read_options_2(&mut self) -> Result<ReadOptions2, RW::Error> {
        Ok(ReadOptions2::new(self.get_u16(READ_OPTIONS_2)?))
    }

    /// ///This register is used to control many aspects of the readout of the sensor. Data format: dd01 0dd1 d00d d10d
    pub fn set_read_options_2(&mut self, value: &ReadOptions2) -> Result<(), RW::Error> {
        self.set_u16(READ_OPTIONS_2, value.value)
    }

    /// ///Even row, even column-default = 0x08 (8) = 1x gain.
    /// Data format: 0000 0000 0ddd dddd
    pub fn get_even_row_even_column_gain(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(EVEN_ROW_EVEN_COLUMN_GAIN)
    }

    /// ///Even row, even column-default = 0x08 (8) = 1x gain.
    /// Data format: 0000 0000 0ddd dddd
    pub fn set_even_row_even_column_gain(&mut self, value: u16) -> Result<(), RW::Error> {
        self.set_u16(EVEN_ROW_EVEN_COLUMN_GAIN, value)
    }

    /// ///Odd row, even column-default = 0x08 (8) = 1x gain.
    /// Data format: 0000 0000 0ddd dddd
    pub fn get_odd_row_even_column_gain(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(ODD_ROW_EVEN_COLUMN_GAIN)
    }

    /// ///Odd row, even column-default = 0x08 (8) = 1x gain.
    /// Data format: 0000 0000 0ddd dddd
    pub fn set_odd_row_even_column_gain(&mut self, value: u16) -> Result<(), RW::Error> {
        self.set_u16(ODD_ROW_EVEN_COLUMN_GAIN, value)
    }

    /// ///Even row, odd column-default = 0x08 (8) = 1x gain.
    /// Data format: 0000 0000 0ddd dddd
    pub fn get_even_row_odd_column_gain(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(EVEN_ROW_ODD_COLUMN_GAIN)
    }

    /// ///Even row, odd column-default = 0x08 (8) = 1x gain.
    /// Data format: 0000 0000 0ddd dddd
    pub fn set_even_row_odd_column_gain(&mut self, value: u16) -> Result<(), RW::Error> {
        self.set_u16(EVEN_ROW_ODD_COLUMN_GAIN, value)
    }

    /// ///Odd row, odd column-default = 0x08 (8) = 1x gain.
    /// Data format: 0000 0000 0ddd dddd
    pub fn get_odd_row_odd_column_gain(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(ODD_ROW_ODD_COLUMN_GAIN)
    }

    /// ///Odd row, odd column-default = 0x08 (8) = 1x gain.
    /// Data format: 0000 0000 0ddd dddd
    pub fn set_odd_row_odd_column_gain(&mut self, value: u16) -> Result<(), RW::Error> {
    ...
    pub fn get_test_data(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(TEST_DATA)
    }

    /// ///The value used to produce a test pattern in "Use Test Data" mode (Reg0x07 bit 6).
    /// Data format: 0000 dddd dddd dd00
    pub fn set_test_data(&mut self, value: u16) -> Result<(), RW::Error> {
        self.set_u16(TEST_DATA, value)
    }

    /// ///Global gain-default = 0x08 (8) = 1x gain. This register can be used to set all four gains at once.
    /// Data format: 0000 0000 0ddd dddd
    pub fn get_global_gain(&mut self) -> Result<u16, RW::Error> {
        self.get_u16(GLOBAL_GAIN)
    }

    /// ///Global gain-default = 0x08 (8) = 1x gain. This register can be used to set all four gains at once.
    /// Data format: 0000 0000 0ddd dddd
    pub fn set_global_gain(&mut self, value: u16) -> Result<(), RW::Error> {
        self.set_u16(GLOBAL_GAIN, value)
    }

    /// ///Data format: dddd dddd d0dd dddd
    pub fn get_cal_threshold(&mut self) -> Result<CalThreshold, RW::Error> {
        Ok(CalThreshold::new(self.get_u16(CAL_THRESHOLD)?))
    }

    /// ///Data format: dddd dddd d0dd dddd
    pub fn set_cal_threshold(&mut self, value: &CalThreshold) -> Result<(), RW::Error> {
    ...
    pub fn get_even_row_even_column_analog_offset(&mut self) -> Result<u16, RW::Error> {
    ...
    /// two's complement, if bit 8 = 1, Offset = bits [0:7] - 256.
    /// Data format: 0000 000d dddd dddd
    pub fn set_even_row_even_column_analog_offset(&mut self, value: u16) -> Result<(), RW::Error> {
    ...
    /// two's complement, if bit 8 = 1, Offset = bits [0:7] - 256.
    /// Data format: 0000 000d dddd dddd
    pub fn get_odd_row_odd_column_analog_offset(&mut self) -> Result<u16, RW::Error> {
    ...
    /// two's complement, if bit 8 = 1, Offset = bits [0:7] - 256.
    /// Data format: 0000 000d dddd dddd
    pub fn set_odd_row_odd_column_analog_offset(&mut self, value: u16) -> Result<(), RW::Error> {
        self.set_u16(ODD_ROW_ODD_COLUMN_ANALOG_OFFSET, value)
    }

    /// ///Data format: d00d d100 1001 1ddd
    pub fn get_cal_ctrl(&mut self) -> Result<CalCtrl, RW::Error> {
        Ok(CalCtrl::new(self.get_u16(CAL_CTRL)?))
    }

    /// ///Data format: d00d d100 1001 1ddd
    pub fn set_cal_ctrl(&mut self, value: &CalCtrl) -> Result<(), RW::Error> {
    ...
    /// two's complement, if bit 8 = 1, Offset = bits [0:7] - 256.
    /// Data format: 0000 000d dddd dddd
    pub fn get_even_row_odd_column_analog_offset(&mut self) -> Result<u16, RW::Error> {
    ...
    /// two's complement, if bit 8 = 1, Offset = bits [0:7] - 256.
    /// Data format: 0000 000d dddd dddd
    pub fn set_even_row_odd_column_analog_offset(&mut self, value: u16) -> Result<(), RW::Error> {
    ...
    /// two's complement, if bit 8 = 1, Offset = bits [0:7] - 256.
    /// Data format: 0000 000d dddd dddd
    pub fn get_odd_row_even_column_analog_offset(&mut self) -> Result<u16, RW::Error> {
    ...
    /// two's complement, if bit 8 = 1, Offset = bits [0:7] - 256.
    /// Data format: 0000 000d dddd dddd
    pub fn set_odd_row_even_column_analog_offset(&mut self, value: u16) -> Result<(), RW::Error> {
    ...
    pub fn get_chip_enable(&mut self) -> Result<u16, RW::Error> {
    ...
    /// a new frame.
    /// Data format: 0000 0000 0000 00dd
    pub fn set_chip_enable(&mut self, value: u16) -> Result<(), RW::Error> {
