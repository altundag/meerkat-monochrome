...
if __name__ == "__main__":
    ...
    output_path = (
        Path(args.input.with_suffix(".png").name)
        if args.output is None
        else args.output
    )

    words = np.fromfile(args.input, dtype="<u4")
    ...
    image = samples.reshape((args.height, args.width))

    Image.fromarray(image, "I;16").save(output_path)
