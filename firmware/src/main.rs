...
async fn main(_spawner: Spawner) {
    ...
    let image_counter = if let Ok(v) = fram.read_u16(0).and_then(|v| {
        fram.write_u16(0, v.wrapping_add(1))?;
        Ok(v)
    }) {
        v
    } else {
    ...
    let sensor_clock_divider =
        FixedU32::<U16>::from_num(clocks::clk_sys_freq() as f32 / sensor::SENSOR_FREQUENCY as f32);
    ...
    let (_, image, _) = unsafe { image_buffers[0].align_to::<u8>() };
