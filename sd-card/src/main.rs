...
fn main() -> ! {
    ...
    let spi = hal::Spi::<_, _, 8>::new(pac.SPI1).init(
        &mut pac.RESETS,
    ...
    let mut sd_controller = Controller::new(block, DummyTimesource::default());
    unsafe {
        STAT_CODE = b'c';
    }
    ...
    match sd_controller.open_file_in_dir(
        ...
        Ok(mut file) => {
            for _ in 0..4 {
                sd_controller
                    .write(&mut volume, &mut file, b"hello!")
                    .unwrap();
            }
